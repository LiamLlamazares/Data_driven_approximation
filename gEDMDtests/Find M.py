...
while error > 0.3:
    ...
    print("error is " + str(error) + " for " + str(datapoints) +
          " data points")
...
plt.loglog(
    datapointslist,
    np.power(datapointslist, -1 / 2) * errorlist[0] /
    np.power(datapointslist[0], -1 / 2))
#We also log-log plot the line with log-log slope -1 starting at the first error and datapoint
plt.loglog(
    datapointslist,
    np.power(datapointslist, -1.0) * errorlist[0] /
    np.power(datapointslist[0], -1.0))
